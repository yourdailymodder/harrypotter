package com.minecraftserverzone.harrypotter.worldgen.structures;
...
public class HiddenBasementStructure extends Structure {

    // A custom codec that changes the size limit for our code_structure_sky_fan.json's config to not be capped at 7.
    // With this, we can have a structure with a size limit up to 30 if we want to have extremely long branches of pieces in the structure.
    public static final Codec<HiddenBasementStructure> CODEC = RecordCodecBuilder.<HiddenBasementStructure>mapCodec(instance ->
            instance.group(HiddenBasementStructure.settingsCodec(instance),
            ...
            ).apply(instance, HiddenBasementStructure::new)).codec();
    ...
    public HiddenBasementStructure(Structure.StructureSettings config,
    ...
    public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext context) {
        ...
        if (!HiddenBasementStructure.extraSpawningChecks(context)) {
    ...
    public StructureType<?> type() {
        return Registrations.HIDDEN_BASEMENT.get(); // Helps the game know how to turn this structure back to json to save to chunks
    }
}